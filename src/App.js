...
export function App() {
  ...
  return (
    <Canvas shadows orthographic camera={{ position: [20, 20, 20], zoom: 100 }} gl={{ preserveDrawingBuffer: true }}>
      <color attach="background" args={['#f2f2f5']} />
      {/** The text and the grid */}
      <Text config={config} rotation={[-Math.PI / 2, 0, -2.2]} position={[-2, -1, -1]}>
        {text}
      </Text>
      
      <Text3D letterSpacing={-0.06} height={0.05}size={0.4} font="/Inter_Medium_Regular.json"  rotation={[-Math.PI / 2, 0, Math.PI*2]} position={[3, -1, 1]}>
          education
          <meshStandardMaterial color="gray" />
        </Text3D>
        <Text3D letterSpacing={-0.00} height={0.05}size={0.2} font="/Inter_Medium_Regular.json"  rotation={[0, 0, 1.55]} position={[-2, -1, 1]}>
          AEC Tech
          <meshStandardMaterial color="black" />
        </Text3D>
        <Text3D letterSpacing={-0.00} height={0.05}size={0.2} font="/Inter_Medium_Regular.json"  rotation={[0, Math.PI, 1.55]} position={[1, -1, -.2]}>
          Metaverse
          <meshStandardMaterial color="black" />
        </Text3D>
        <Text3D letterSpacing={-0.00} height={0.05}size={0.2} font="/Inter_Medium_Regular.json"  rotation={[0, 3.9, 1.55]} position={[-1.5, -.5, -2]}>
          Web3
          <meshStandardMaterial color="black" />
        </Text3D>
        <Text3D letterSpacing={-0.00} height={0.05}size={0.2} font="/Inter_Medium_Regular.json"  rotation={[0, 3.9, 1.55]} position={[-1.8, -1, -1.8]}>
          Blockchain
          <meshStandardMaterial color="black" />
        </Text3D>
